...
public class EmployeeController implements EmployeeService{
    ...
    public ObservableList<Employee> getAllEmployee() {
        ...
        try {
            ...
            while(res.next()){
                employeeObservableList.add(new Employee(res.getString(1),res.getString(2),res.getString(3),res.getString(4), res.getString(5)));
            }
    ...
    public boolean addEmployee(Employee employee) {
        try {
            ...
            stm.setObject(5, employee.getPassword());
    ...
    public boolean updateEmployee(Employee employee) {
        try {
            PreparedStatement stm = DBConnection.getInstance().getConnection().prepareStatement("UPDATE employee SET name = ?, company = ?, email = ?, password = ? WHERE id = ?");
            stm.setObject(1, employee.getName());
            stm.setObject(2, employee.getCompany());
            stm.setObject(3, employee.getEmail());
            stm.setObject(4, employee.getPassword());
    ...
    public Employee searchEmployee(String id) {
        try {
            ...
            if (res.next()) {
                return new Employee(res.getString(1), res.getString(2), res.getString(3), res.getString(4), res.getString(5));
            }
            return null;
        } catch (SQLException e) {
            throw new RuntimeException(e);
